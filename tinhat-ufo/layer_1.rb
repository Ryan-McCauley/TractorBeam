...
target = 'https://www.reddit.com/r/ufos/new/.json'
tag = 'L1::UFO::'
...
FileUtils.mkdir_p('json_data/ufo')
FileUtils.mkdir_p('master_list/ufo')
FileUtils.mkdir_p('detected_changes/ufo')
...
if response.success?
  ...
  json_filename = "json_data/ufo/#{tag}#{timestamp}.json"
  html_filename = "master_list/ufo/#{tag}#{timestamp}.html"
  ...
  File.open(html_filename, 'w') do |result_file|
    result_file.puts <<~HTML
      ...
      <html>
      <head>
        <title>"UFO Sightings from r/ufos #{timestamp}"</title>
        <link rel="stylesheet" type="text/css" href="style.css">
      </head>
      <body>
        <h1>UFO Sightings from r/ufos</h1>
