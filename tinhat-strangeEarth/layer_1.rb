...
target = 'https://www.reddit.com/r/strangeEarth/new/.json'
tag = 'L1::STRANGEEARTH::'
...
FileUtils.mkdir_p('json_data/strange_earth')
FileUtils.mkdir_p('master_list/strange_earth')
FileUtils.mkdir_p('detected_changes/strange_earth')
...
if response.success?
  ...
  json_filename = "json_data/strange_Earth/#{tag}#{timestamp}.json"
  html_filename = "master_list/strange_Earth/#{tag}#{timestamp}.html"
  ...
  File.open(html_filename, 'w') do |result_file|
    result_file.puts <<~HTML
      ...
      <html>
      <head>
        <title>"Data Collected from r/StrangeEarth @ #{timestamp}"</title>
