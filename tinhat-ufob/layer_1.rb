...
target = 'https://www.reddit.com/r/ufob/new/.json'
tag = 'L1::UFOB::'
...
FileUtils.mkdir_p('json_data/ufob')
FileUtils.mkdir_p('master_list/ufob')
FileUtils.mkdir_p('detected_changes/ufob')
...
def extract_keywords(posts)
  ...
  most_common_words = word_count.sort_by { |_word, count| -count }.map(&:first).take(33)
  most_common_words
end
...
if response.success?
  ...
  json_filename = "json_data/ufob/#{tag}#{timestamp}.json"
  html_filename = "master_list/ufob/#{tag}#{timestamp}.html"
  ...
  File.open(html_filename, 'w') do |result_file|
    result_file.puts <<~HTML
      ...
      <html>
      <head>
        <title>"UFOB Sightings from r/ufob #{timestamp}"</title>
        <link rel="stylesheet" type="text/css" href="style.css">
      </head>
      <body>
        <h1>UFOB Sightings from r/ufos</h1>
